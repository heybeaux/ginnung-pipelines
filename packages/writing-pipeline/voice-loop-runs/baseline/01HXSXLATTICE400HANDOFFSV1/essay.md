...
Here's the part that should bother you more than the headline number: the MAST paper found that using better models improved success rates by less than 5%. Swapping in a stronger LLM barely changed the outcome. The failures aren't happening because the models are too dumb. They're happening because the coordination between agents is broken — agents hand off malformed state, downstream agents accept it silently, and the pipeline returns 200 OK with garbage at the end. No crash. No error. Just confidently wrong output that looks fine until a human reads it.
...
The topline: 93.0% pass rate. 372 of 400 handoffs validated successfully. Zero pipelines crashed. But that topline obscures what's actually interesting in the per-tier breakdown.

---

L1 passed 100% of handoffs. Every single one. This makes sense — L1 is structural, and the agents in Forge are well-prompted enough to return valid JSON with the right fields. L1's job is to be a fast gate that catches the obvious stuff, and in a mature pipeline, the obvious stuff is already handled. L1 justified its place in the tier stack not by catching failures but by costing nothing and confirming structure in under 200ms.
...
What's next: I'm building an observability dashboard — a web UI for the audit trail, so you can see handoff state, validation results, confidence distributions, and escalation patterns without grepping JSONL files. After that, I want to run this benchmark across at least three more pipeline types — code review, data extraction, customer support drafting — at 1,000+ handoffs each, and publish the same level of raw data.
